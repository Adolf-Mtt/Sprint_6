...
def positive_assert(kit_name):
    ...
    assert str(response.json()["id"]) in var.text and response.json()["name"] in var.text.replace("\\","") # Valor esperado = True / Positivo
...
def negative_assert_code_400(kit_name):
    ...
    # Solicitud->GET-> "var" se compara con la anterior Solicitud->POST-> "response"
    var = sender_stand_request.get_kit_table()
    assert str(kit_name) not in var.text # Valor esperado = True / Positivo
